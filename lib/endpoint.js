function make() {
  ...
  router.get('/ip', function () {
    var response = {
      status: 'OK',
      data: { ip: this.req.connection.remoteAddress }
    };
  ...
  router.delete('/delete', function () {
    ...
    db.get(body.hostname, function (err, value) {
      if (err) return self.res.end(JSON.stringify({ status: 'ERROR', message: err.message }));
      if (value && value.password === body.password) {
        db.del(body.hostname, function (err, value) {
          if (err) return self.res.end(JSON.stringify({ status: 'ERROR', message: err.message }));
          self.res.end(JSON.stringify(response));
        });
      } else {
  ...
  router.post('/update', function () {
    ...
    db.get(body.hostname, function (err, value) {
      if (err) return self.res.end(JSON.stringify({ status: 'ERROR', message: err.message }));
      if (value && value.password === body.password) {
        value.ip = body.ip || self.req.connection.remoteAddress;
  ...
  router.post('/register', function () {
    ...
    db.get(body.hostname, function (err, value) {
      ...
      if (value === undefined) {
        var ip = self.req.connection.remoteAddress;
